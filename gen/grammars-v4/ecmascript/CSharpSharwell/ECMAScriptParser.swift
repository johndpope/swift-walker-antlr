...
open class ECMAScriptParser: Parser {
	...
	enum Tokens: Int {
		case EOF = -1, RegularExpressionLiteral = 1, LineTerminator = 2, OpenBracket = 3, 
                 ...
                 Equals_ = 31, NotEquals = 32, IdentityEquals = 33, IdentityNotEquals = 34, 
                 ...
                 MultiLineComment = 101, SingleLineComment = 102, HtmlComment = 103, 
				...
				if ( _alt==1 ) {
					...
					case 7:
						...
						if (!(//closure
						 { () -> Bool in
						      let testSet: Bool = {  () -> Bool in
						   let testArray: [Int] = [_la, ECMAScriptParser.Tokens.Equals_.rawValue,ECMAScriptParser.Tokens.NotEquals.rawValue,ECMAScriptParser.Tokens.IdentityEquals.rawValue,ECMAScriptParser.Tokens.IdentityNotEquals.rawValue]
						    return  Utils.testBitLeftShiftArray(testArray, 0)
						}()
						      return testSet
						 }())) {
	 ...
	 open func getter() throws -> GetterContext {
		...
		do {
		 	...
		 	if (!(_input.Lt(1).Text.Equals("get"))) {
		 	    throw ANTLRException.recognition(e:FailedPredicateException(self, "_input.Lt(1).Text.Equals(\"get\")"))
		 	}
		 	setState(606)
		 	try match(ECMAScriptParser.Tokens.Identifier.rawValue)
		 	setState(607)
	 ...
	 open func setter() throws -> SetterContext {
		...
		do {
		 	...
		 	if (!(_input.Lt(1).Text.Equals("set"))) {
		 	    throw ANTLRException.recognition(e:FailedPredicateException(self, "_input.Lt(1).Text.Equals(\"set\")"))
		 	}
		 	setState(610)
		 	try match(ECMAScriptParser.Tokens.Identifier.rawValue)
		 	setState(611)
		...
		do {
		 	...
		 	case 4:
		 		...
		 		if (!(_input.Lt(1).Type == CloseBrace)) {
		 		    throw ANTLRException.recognition(e:FailedPredicateException(self, "_input.Lt(1).Type == CloseBrace"))
		 		}
